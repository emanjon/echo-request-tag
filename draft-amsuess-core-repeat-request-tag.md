...
The Request-Tag is intended for use as a resource-local identifier for differentiating between representations of the same resource provided in a request. In case of blockwise transfers ({{RFC7959}}), the Request-Tag enables the receiving CoAP server to distinguish between request payloads (blocks) belonging to different message bodies having the same block number.
