...
The value of the Request-Tag option MUST be different from other Request-Tag values used by the CoAP client in any other concurrent blockwise request operations to the same resource.  It is generated by the CoAP client providing the resource, and may be reused when the operation is concluded. (Clients are encouraged to generate compact values) 

The Request-Tag value of an initiated blockwise request operation is chosen by the client in such a way that a tag value is not equal to any other operation that is not concluded. What constitutes a concluded operation depends on the application, and is outlined individually in {{req-tag-applications}}.
...
If a CoAP server receives a message with a Block1 option having the same Request-Tag value (with or without the Request-Tag option) and block number (NUM, {{RFC7959}}) as a previously received message to the same resource in a blockwise request operation that is not concluded then the CoAP server SHOULD send a (TBD: error code).
